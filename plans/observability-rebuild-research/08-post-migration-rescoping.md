...
| Collector boundary | `copilot-mro/deployment/otel/base.yaml:1-31` owns the OTLP receivers; `:32-210` owns identity, content, cardinality, redaction/filtering and batching. Backend overlays own complete pipelines. | No production `file_storage`, `sending_queue` or explicit exporter `retry_on_failure` configuration exists. Add these only in the production durability work. |
| Destination overlays | `backend-oss.yaml` routes to Tempo, Prometheus and Loki. `backend-aws.yaml` routes traces, metrics and logs to X-Ray/CloudWatch using workload identity. `backend-azure.yaml:1-11` is explicitly configuration-validated but not deployed. | There is no `backend-newrelic.yaml`. AWS/Azure/New Relic are not production-supported until retrieved canaries pass; Azure also needs the Phase 8.5 supportability decision. |
...
| 8.5 Destination readiness | OSS/AWS/Azure overlays exist; New Relic and durable queues are absent. AWS/Azure live support remains unproven/currently deferred. | Configuration work may start; live claims require credentials and owner go-ahead. |
...
| 2 | **Implemented as artifacts; production unproven:** OSS/AWS/Azure configs and AWS IaC exist, but provider deployment/live support is deferred. |
