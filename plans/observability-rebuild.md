...
passed. **Task 11.6 is NOT complete — this sentence is corrected 2026-09-20.** It previously read "Task 11.6 is complete within the bounded non-container review scope", and three documents contradict that reading: this task's own detail plan (`observability-rebuild-phase-11-audit-followups.md`) titles the same run **partial** and ends *"do not claim product-event database replay or backfill idempotency from this run"*; `observability-close-out-gate.md` enumerates **15 open items, 4 of them unprovable without a live stack**; and the blocker named in this very sentence was never cleared. What IS true is that the bounded non-container lane passed. The diagnosis has since moved on: the scratch-DB failure is not two checks but **the whole lane** (7 failed / 232 errors), and its cause is that `PROVISION_SEQUENCE` declares provisioning as the migration **plus** `provision_rls.py` while `run_db_lane.py` runs only the first half. Detail: the product-event replay scratch DB lane still blocks before assertions on
