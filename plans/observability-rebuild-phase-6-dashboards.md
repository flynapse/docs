...
- **D6 — dark-panel convention.** ~~Every panel or rule whose source series does not exist yet carries a description beginning `DARK until Stream L:` (or `DARK until Stream F cut-over:`) naming its source metric. The guard test enforces it mechanically: any dashboard target or rule expression naming an `agent_*`, `gen_ai_*` or `claude_code_*` series must carry the Stream L note; any `browser.*` reference the Stream F note.~~ **RETIRED 2026-09-20.** D6 had two states and a fixed string, and both failed once something was actually emitted: a panel on a newly-real series was forced to keep a note saying it was dark, and a rule naming a series in neither hand-maintained tuple fell through both and was checked by nothing. It is replaced by a three-state grammar derived from the emitter — `DARK until <what would arm it>` / `WIRED <YYYY-MM-DD>, retrieval unproved: <what emits it>` / `LIVE since <probe> (<YYYY-MM-DD>)` — enforced by `copilot-mro/tests/integration/otel/_emitted_series.py`, which reads `agent_shared/telemetry.py` by AST and proves per series that the instrument exists, its kind matches its constructor, it is recorded, and its public recorder does or does not have a production call site. `DARK until Stream F cut-over:` survives unchanged; it was always an instance of the general form. Alerts on absent series simply never fire — `promtool` accepts them; nothing errors at runtime.
